...
export class Enemy {
  ...
  update(ts: number): { didShoot: boolean, muzzlePos?: vec3, dir?: vec3 } {
    ...
    // Roaming Logic
    if (this.moveTimer <= 0) {
        this.targetAngle = this.rotation + (Math.random() - 0.5) * Math.PI;
        this.moveTimer = 2.0 + Math.random() * 3.0; // Change direction every 2-5 seconds
        this.throttle = Math.random() > 0.3 ? 1 : -0.5; // Mostly move forward, sometimes backup
    }
    this.moveTimer -= ts / 1000;
    
    // Smooth rotation towards random target
    const PI2 = Math.PI * 2;
    let angleDiff = (this.targetAngle - this.rotation) % PI2;
    ...
    const linVel = UT.VEC3_SCALE(forward, this.throttle * speed);
